...
public class DaySelector extends DateChooserPanel {
   ...
   */
  private Font defaultWeekdayFont = new Font(Display.getCurrent(), 
      "*", 8, SWT.BOLD);
   ...
   */
  private DaySelectorButton_mouseListener dayMouseListener =
    new DaySelectorButton_mouseListener(this);
  ...
  public void daySelectorButton_mouseReleased(MouseEvent mouseEvent) {
    int iDay = -1;
    try {
      iDay = Integer.parseInt( ( (Label) mouseEvent.getSource()).getText().
          trim());
    }
    ...
    for (int iDay = 1; iDay <= lastDay; iDay++) {
      ...
      if (isCurrentMonth && 
        ...
        style.setStyle(thisDay, DaySelectorStyle.TODAYLABEL);
          ...
          iDay == selectedDate.get(Calendar.DAY_OF_MONTH)) {
 ...
 * @version $Revision: 1.2 $
 */
class DaySelectorButton_mouseListener
